...
public class OfferItem {
	...
	public OfferItem(ProductSnapshot product, int quantity, Discount discount,
		...
		this.totalCost = product.getProductPrice()
				.multiply(new Money(new BigDecimal(quantity)))
	...
	public int hashCode() {
		...
		result = prime
				* result
				+ ((getProduct().getProductName() == null) ? 0 : getProduct()
						.getProductName().hashCode());
		result = prime
				* result
				+ ((getProduct().getProductPrice() == null) ? 0 : getProduct()
						.getProductPrice().hashCode());
		result = prime
				* result
				+ ((getProduct().getProductId() == null) ? 0 : getProduct()
						.getProductId().hashCode());
		result = prime
				* result
				+ ((getProduct().getProductType() == null) ? 0 : getProduct()
	...
	public boolean sameAs(OfferItem other, double delta) {
		...
		Money max, min;
		if (totalCost.compare(other.totalCost) > 0) {
			max = totalCost;
			min = other.totalCost;
		} else {
			max = other.totalCost;
		...
		Money acceptableDelta = max.multiply(new Money(new BigDecimal(
				delta / 100)));

		return acceptableDelta.compare(difference) > 0;
	}
